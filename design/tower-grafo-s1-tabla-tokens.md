...
owner: "@diseno"
date: 2026-09-24
prototipo: "design/tower-grafo-s1-lienzo.html — 51.198 caracteres, sha256 e9970382f2b0355a10737d657fa23d31b5d2af154f32786f726db8c31a2d3afb (leído íntegro en esta sesión; read devuelto con sha y truncated=false en el último tramo)"
...
| 3 | Fondo de tarjeta, zona de estado y chip «pedido» | `--color-surface-elevated` #ffffff | exacto | `card.tsx:54` (`bg-card/80`) → `tailwind.config.js:88` → `--card` `theme.css:7` = **el mismo valor que `--background`**: el escritorio no distingue tarjeta de página, y ninguna de las dos es blanco |
...
| 16 | Estado «pedido» (43001) | neutros: `--color-surface-elevated` + `--color-border-default` + `--color-text-primary` | **inexacto**: estado observado sin chip sólido propio; no existe token de «estado neutro» | confirmado en el escritorio: `PanelRow.tsx:18` (`requested: "Solicitado"`), texto dentro de la celda, sin chip ni color. Nota de producto vigente: 43001 sin emisión viva (`architecture/tower-grafo-live-kindcheck.md`, 2026-09-24) |
...
- **Demuestra:** qué token de la foundation usa cada rol de la superficie, auditado sobre mi lectura íntegra del fichero (`e9970382…`, 51.198 caracteres) contra el `foundation_block` devuelto hoy. Re-verificable con `grep` sobre el HTML. Y, desde la pasada (2), a qué `fichero:línea` de `main` (`ab63b0129`) resuelve cada paso de escritorio de las 28 filas —o, en los cuatro que no tienen paso en la foundation, a su valor observado y su hueco—, con el comando y su salida en `reports/tower-grafo-s1-tokens-desktop.md`.
