 ...
 * the row count is the edge count, never a row per parent. Malformed events —
 * no `job`, no `child`, an unreadable timestamp — are dropped rather than
 * guessed at, so one unreadable event cannot blank the section.
 */
export function foldHandoffEdges(events: readonly RelayEvent[]): HandoverRow[] {
  ...
  for (const event of events) {
    ...
    rows.push({
      ...
      child: { jobId: childJobId, name: null },
